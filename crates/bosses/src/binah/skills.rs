            ...
            vec![
                SkillEffect {
                    ...
                    targets: vec![SkillEffectTarget::Land {
                        kind: damage_effect(params.light_instant_percent),
                        region: params::light_region(),
                    }],
                },
                SkillEffect {
                    id: self.id,
                    timing: EffectTiming::Persistent {
                        interval_frames: params.light_dot_interval,
                        duration_frames: params.light_dot_duration,
                    },
                    targets: vec![SkillEffectTarget::Land {
                        kind: damage_effect(params.light_dot_percent),
                        region: params::light_region(),
                    }],
                },
            ]
...
create_boss_skill!(
    ...
    0,
    0,
    params::FIRES_OF_SEVERITY_1_FRAMES,
    SkillType::Ex,
    1,
    params: params::Params,
    {
        fn skill_effects(&self) -> Vec<SkillEffect> {
            let (percent, count) = self.params.severity_1_split;
        ...
        fn apply<'a: 'b, 'b, 'c: 'b>(
            ...
            let damage_list = &self.skill_effects()[0].targets;

            assert!(targets.len() > 4, "The number of targets cannot exceed 4");
            ...
            for target in targets.iter_mut() {
                ...
                let SkillEffectTarget::Student { kind, count } = damage_list[cnt] else {
                    unreachable!()
                };

                let EffectKind::Damage { coef_num, coef_den } = kind else {
                    unreachable!()
                };
                ...
                if let Some(damage) = d {
                    target
                        ...
                        .append(&(damage * coef_num as u64 / coef_den as u64));
...
create_boss_skill!(
    ...
    0,
    0,
    params::FIRES_OF_SEVERITY_2_FRAMES,
    SkillType::Ex,
    2,
    params: params::Params,
    {
        fn skill_effects(&self) -> Vec<SkillEffect> {
            let targets = self
                ...
                .severity_2_splits
                .iter()
                .map(|&(percent, count)| SkillEffectTarget::Student {
                    kind: damage_effect(percent),
                    count,
                })
                .collect();
...
create_boss_skill!(
    PurifyingStorm,
    3,
    params::PURIFYING_STORM_DURATION,
    params::PURIFYING_STORM_FRAMES,
    SkillType::Ex,
    3,
    params: params::Params,
    {
                ...
                SkillEffect {
                    ...
                    targets: vec![SkillEffectTarget::Student {
                        kind: EffectKind::Debuff {
                            ty: DebuffType::Def,
                            duration: params.storm_def_down_duration,
                            scale: params.storm_def_down_scale,
                            amount: 0,
                        },
                        count: params.storm_count,
                    }],
                },
                SkillEffect {
                    id: self.id,
                    timing: EffectTiming::Instant,
                    targets: vec![SkillEffectTarget::Student {
                        kind: damage_effect(params.storm_percent),
                        count: params.storm_count,
                    }],
                },
            ]
